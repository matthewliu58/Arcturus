...
def main():
    ...
    # Run Go test
    print("Running Carousel Greed tests (20 runs)...")
    result = subprocess.run("go test -v -run TestFlowOptimizationSolverMulti -count=1", shell=True, capture_output=True, text=True)
    print(result.stdout)
    if result.stderr:
        print("Errors:", result.stderr)
    
    results_file = "cost266_carousel_greed_results.txt"
    ...
    log_files = sorted(glob.glob("carousel_greed_test_*_*.log"))
    ...
    for i, log_file in enumerate(log_files[:20], 1):
        ...
        result = subprocess.run(f"py carousel-greed-evaluation.py {log_file}", shell=True, capture_output=True, text=True)
