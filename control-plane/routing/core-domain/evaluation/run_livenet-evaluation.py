...
if len(sys.argv) > 1:
    ...
    log_file = "janos-us-ca_livenet_test_multi.log"
...
with open(log_file) as f:
    for line in f:
        ...
        if e:
            key = tuple(sorted([e.group("src"), e.group("tgt")]))  # 无向边
...
for p in selected_paths:
    ...
    for i in range(len(path)-1):
        e = tuple(sorted([path[i], path[i+1]]))  # 无向边
        edge_usage[e] += 1

# =============================
# 4. Hot-edge Risk (HAR)
# =============================
# 超过 CPU 60% 乘以指数惩罚，重复越多越惩罚
HAR_list = []
for p in selected_paths:
    path = p["path"]
    har = 1.0
    for i in range(len(path)-1):
        ...
        if cpu > 60:
            har *= np.exp((cpu - 60)/10) * (1 + (usage_count-1)/5)  # 指数惩罚+重复
        else:
            har *= 1 + (usage_count-1)/10  # 重复也有少量加分
    HAR_list.append(har)
